...
\[L(\lambda,x = (k_{1},k_{2},k_{3}...)) =
\prod_{i=1}^{100} f(k_{i}) \] 
...
\[\log f(\theta|n,y) = \log (\binom{n}{y}\theta^{y}(1-\theta)^{(n-y)})\]

We break up the product using the log rule $\log(ab)=\log(a)+\log(b)$:

\[\log f(\theta|n,y) = \log\binom{n}{y}+ \log\theta^{y}+\log(1-\theta)^{(n-y)})\]

We bring down the exponents using the $\log(a^{b})=b\log(a)$ rule:

\[\log f(\theta|n,y) = \log\binom{n}{y}+ y\log\theta+(n-y)\log(1-\theta)\]
